...
type SyntaxEntry = { language: string; source: string; lines: SyntaxLines };
...
export function highlightedLines(language: string, source: string): SyntaxLines {
  ...
  return entriesBySource.get(`${language}\u0000${normalized}`) ?? normalized.split("\n").map((line) => [{ content: line, color: "var(--atom-syntax-foreground)" }]);
}
